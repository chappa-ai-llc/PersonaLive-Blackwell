        ...
        try:
            ...
            for name in inputs:
                ...
                if isinstance(hinput, torch.Tensor):
                    target_dtype_np = trt.nptype(self.input_dtypes[name])
                    target_dtype_torch = numpy_to_torch_dtype(target_dtype_np)
                    if hinput.dtype != target_dtype_torch:
                        hinput = hinput.to(target_dtype_torch)
                # ------------------------

                # Calculate REAL size of the current input
                real_nbytes = 0
                if isinstance(hinput, torch.Tensor):
                ...
                if (isinstance(hinput,torch.Tensor) and hinput.device.type=="cuda" and hinput.device.index==self.device_int):
                    ...
                    # FIX: Use real_nbytes instead of self.input_nbytes[name]
                    cuda.memcpy_dtod_async(self.dinputs[name], ptr, real_nbytes, self.stream)
                else:
    ...
    def prefill(self, skip_check=SKIP_ENGINE_MODEL_CHECK, **inputs):
        ...
        try:
            for name in inputs:
                ...
                # --- AUTO-CASTING FIX FOR PREFILL ---
                if isinstance(hinput, torch.Tensor):
                    # Determine target dtype based on whether it is an input or output tensor
                    if in_input:
                        target_dtype_np = trt.nptype(self.input_dtypes[name])
                    else:
                        target_dtype_np = trt.nptype(self.output_dtypes[name])
