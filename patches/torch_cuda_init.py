...
def _check_capability():
    ...
    CUDA_ARCHES_SUPPORTED = {
        "12.6": {"min": 50, "max": 90},
        "12.8": {"min": 70, "max": 120},
        "13.0": {"min": 75, "max": 130}, # Patch: Bumped max to 130
    }

    if (
        torch.version.cuda is not None and torch.cuda.get_arch_list()
    ):  # on ROCm we don't want this check
        # PATCH: Use torch._C._cuda_getDeviceCount() instead of device_count()
        # This prevents crashing if NVML detects more devices than the initialized CUDA runtime supports.
        runtime_count = torch._C._cuda_getDeviceCount()
        for d in range(runtime_count):
            ...
            # --- PATCH START: Allow GB10/CUDA 13.0/12.x+ ---
            # Unconditionally allow newer architectures to support 12.1+ hardware
            max_arch = max(max_arch, 130)
            # --- PATCH END ---
